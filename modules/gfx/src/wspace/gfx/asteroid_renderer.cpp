...
void AsteroidRenderer::clear_instances()
{
    _instances.clear();
}

void AsteroidRenderer::add_instance(const Asteroid &instance)
{
...
void AsteroidRenderer::update()
{
    ...
    glVertexAttribPointer(loc_in_position, 2, GL_FLOAT, GL_FALSE, sizeof(Asteroid),
                          reinterpret_cast<const void *>(std::uintptr_t(offsetof(Asteroid, position))));

    _instance_count = _instances.size();
}

void AsteroidRenderer::draw(const glm::mat4 &mvp)
{
    _program.use();
...
auto Data::vertices() -> std::span<const Vertex>
{
    static constexpr std::initializer_list<const Vertex> vertices = {
        {.position = glm::vec2(+0, -1), .color = glm::vec4(1, 1, 0, 1)},
        {.position = glm::vec2(+1, +0), .color = glm::vec4(0, 1, 1, 1)},
        {.position = glm::vec2(+0, +1), .color = glm::vec4(1, 0, 1, 1)},
        {.position = glm::vec2(-1, +0), .color = glm::vec4(1, 1, 1, 1)},
    };
    return {vertices};
}
