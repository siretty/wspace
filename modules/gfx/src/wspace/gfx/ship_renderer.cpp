...
void ShipRenderer::clear_instances()
{
    _instances.clear();
}

void ShipRenderer::add_instance(const Ship &ship_instance)
{
    _instances.emplace_back(ship_instance);
}
...
void ShipRenderer::update()
{
    ...
    glVertexAttribPointer(loc_in_position, 2, GL_FLOAT, GL_FALSE, sizeof(Ship),
                          reinterpret_cast<const void *>(std::uintptr_t(offsetof(Ship, position))));

    glEnableVertexAttribArray(loc_in_angle);
    glVertexAttribDivisor(loc_in_angle, 1);
    glVertexAttribPointer(loc_in_angle, 1, GL_FLOAT, GL_FALSE, sizeof(Ship),
                          reinterpret_cast<const void *>(std::uintptr_t(offsetof(Ship, angle))));

    _instance_count = _instances.size();
}

void ShipRenderer::draw(const glm::mat4 &mvp)
{
    _program.use();
...
auto Data::vertices() -> std::span<const Vertex>
{
    static constexpr std::initializer_list<const Vertex> vertices = {
        ...
        {.position = glm::vec2(+1, +0), .color = glm::vec4(0, 1, 0, 1)},
        {.position = glm::vec2(-1, -1), .color = glm::vec4(0, 0, 1, 1)},
    };
    return {vertices};
}
...
{
    static constexpr std::initializer_list<const GLushort> indices = {0, 1, 2, 0};
    return {indices};
}
...
mat2 make_rotation(float angle) {
    float s = sin(angle);
    float c = cos(angle);
...
void main() {
    mat2 rotation = make_rotation(in_angle);

    gl_Position = un_mvp * vec4(rotation * 0.1 * va_position + in_position, -30.0, 1.0);
    vertex_color = va_color;
    vertex_color.r = in_angle;
}
)glsl"};
